<x-layouts.app title="Periksa Pasien">
    ...
    <div class="container-fluid px-4 mt-4">
        ...
        {{-- ✅ TAMBAH: tampilkan error validasi / stok habis --}}
        @if ($errors->any())
        ...
        {{-- ✅ opsional: kalau controller pakai session success --}}
        @if (session('success'))
                    ...
                    <div class="card-body">
                        <form action="{{ route('periksa-pasien.store') }}" method="POST">
                                ...
                                <select id="select-obat" class="form-select">
                                    ...
                                    @foreach ($obats as $obat)
                                        <option value="{{ $obat->id }}"
                                            ...
                                            {{ (($obat->stok ?? 0) <= 0) ? 'disabled' : '' }}>
                                            {{-- ✅ tambah info stok + indikator menipis --}}
                                            {{ $obat->nama_obat }}
                                            - Rp{{ number_format($obat->harga) }}
                                            (Stok: {{ $obat->stok ?? 0 }})
                                            @if (($obat->stok ?? 0) > 0 && ($obat->stok ?? 0) <= 5)
                                                ⚠ menipis
                            ...
                            <div class="form-group mb-3">
                                ...
                                {{-- ✅ opsional kecil: helper text --}}
                                <small class="text-muted">
                                    Obat stok 0 otomatis tidak bisa dipilih.
                                </small>
                            </div>
...
<script>
    ...
    selectObat.addEventListener('change', () => {
        ...
        const stok = parseInt(selectedOption.dataset.stok || 0); // ✅ tambah stok

        // ✅ cegah pilih kosong / duplikat
        if (!id || daftarObat.some(o => o.id == id)) {
            return;
        }

        // ✅ handling stok habis di sisi UI (nilai plus)
        if (stok <= 0) {
            alert(`Stok obat "${nama}" habis. Silakan pilih obat lain.`);
